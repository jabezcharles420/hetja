...
competitor and the substrate**, so build SOS/notification and photo-reporting on the WhatsApp
...
consumables**, so plan 1/yr replacement and a "sighting without collar" (visual-ID) path.
...
interviews, temperament tests, vaccinations before rehoming), and Hetja should digitise that
...
- **What to add:** FCM/Web Push primary + SMS fallback for SOS (SMS already appears in the
  ...
  and a "SOS silenced by the OS" detection, since the 8-min escalation must not depend on push being
  delivered.
...
- **Design consequences:** (1) budget collar replacement at ~1/yr and a `retag` scan type already
  in the enum; (2) the network must function when the collar is gone, via a photo + `cv_embedding`
...
- Internet is not guaranteed (power cuts, communal shutdowns, network fatigue), so every
...
An HMAC with a fixed pepper is deterministic per phone and linkable in practice, and under the DPDP
...
government impose localization on sensitive/critical classes, so geolocation-heavy registers should
...
- **Recognition is personal at small scale.** In a city like Mumbai the feeder community is
  thousands, not millions, so individual recognition ("Feeder of the ward month", a named thank-you
...
4. **Quality-weighted trust, non-punitive.** Weight `trust_events` by *validated* scans (AI or
   ...
   score drop look like shaming; frame it as "needs verification".
