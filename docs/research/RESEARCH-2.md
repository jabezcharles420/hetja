...
- **Rate limiting under CGNAT is a key-choice problem, not a plugin problem.** `@fastify/rate-limit`
  defaults to an IP-based key with a `/64` IPv6 mask, which is exactly wrong for Jio/Airtel pools that share
...
- **Postgres partitioning that preserves `client_uuid` idempotency is a hash-partition-by-`client_uuid`
  ...
  allowed across a partitioned table in PG 15, so they are a dead end for the global guarantee. The
...
- **Push on Jio/Airtel CGNAT is layered, and only SMS is a guarantee.** No inbound sockets means no
  ...
  ~₹35–60 K/yr (~$450–700), which is affordable, and free-form/utility is untaxed inside the 24 h customer
  service window.
...
- **YOLOv8n is the right CPU default; `-s` buys little for dog/food presence.** Official Ultralytics
  ...
  at 5 M/yr is far under what 2 vCPU INT8 sustains (~260 K–700 K images/day), so CPU-only is fine for
...
- **Route-level config** (`config: { rateLimit: {...} }`) lets you keep reads loose and make writes
  ...
  headers.
- **Store choice vs the repo's no-Redis rule:** the default store is an in-memory LRU, which is correct and
...
- Explicit paths (no wildcards) cost ~2% of `JSON.stringify`; wildcards cost ~50%, so enumerate the
...
- The **worker** (`apps/worker/src/index.ts`) has no signal handling: a `SIGTERM` mid-poll leaves
  ...
  `SIGTERM` handling that stops the loop (jobs are safe because `locked_until` expires) and closes the pool.
- DoS posture: set `requestTimeout`, `connectionTimeout`, `maxRequestsPerSocket`, and `handlerTimeout`
  (Fastify 5's cooperative timeout with `request.signal`). `bodyLimit` stays 1 MiB by default, which is workable
...
- Fastify is I/O-bound and async; a single Node process will idle the second vCPU once CPU-bound work
  ...
  yr ≈ 14 K/day ≈ <0.2 req/s average), so a **single process is correct for the pilot**, with the AI worker
...
- `@fastify/under-pressure` (core plugin): 503 when event-loop delay/heap/RSS exceed thresholds;
  it complements `return503OnClosing` so a slow DB doesn't wedge the box.
...
- **Why 16 partitions:** PG's own guidance is to keep partition counts modest (a few hundred is fine;
  thousands hurt planning). ~5 M rows/yr over 16 partitions ≈ ~300 K rows/partition/yr, so indexes stay
...
- **Migration cost (be honest about it):** you cannot convert a plain table into a partitioned one.
  ...
  rows/yr at Phase 0–1", but the trigger to execute is index size / bloat, not row count alone
...
- `ai_validation` per scan accumulates detections. Discipline: store a **compact summary** in JSONB
  ...
  is fine for point lookups but penalises seq scans, so keep heatmap/recent queries projecting only the
  columns they need.
...
- **HNSW is the default.** Better speed-recall trade-off, **no training step and no re-clustering**,
  ...
  float32 ≈ 307 MB of vectors; an HNSW index roughly doubles that, which fits in RAM on the 2-vCPU box,
...
- Build with `CREATE INDEX CONCURRENTLY ... USING hnsw (cv_embedding vector_ip_ops) WITH (m=16,
  ...
  (higher = recall, slower). At ≤~50 K rows an exact scan is already sub-100 ms warm, so **start exact,
...
- **Batching helps throughput, not latency**: batch 8–16 in the worker (the poll loop already batches
  ...
  Set `OMP_NUM_THREADS`/`ORT_NUM_THREADS = 2` and **pin**, because the box also runs the API and Postgres
...
1. **Pick the frozen descriptor, then the column.** `cv_embedding` is `VECTOR(768)`: CLIP ViT-L/14
   ...
   TYPE vector(512)`. Decide the descriptor first (MegaDescriptor if available, since it beats CLIP
...
6. **DPDP note (ties to RESEARCH-1 §3):** embeddings are reversible-ish fingerprints of a specific
   animal at a known spot, so treat the embedding table as personal-adjacent data; it's in scope for E4's
